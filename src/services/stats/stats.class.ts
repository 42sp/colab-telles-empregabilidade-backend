...
export class StudentsStats {
	...
	async find(params?: StudentsStatsParams): Promise<StatsResult> {
		const total_students : any = await this.Model('students').count('id', { as: 'total' }).first();
		const working_students : any = await this.Model('students').where('working', true).count('id', { as: 'total' }).first();
		const avgCompensationResult: any = await this.Model('students').whereRaw('"compensation" > 0').sum('compensation as sum').count('compensation as count').first();
		const avgCompensation = avgCompensationResult.count > 0 ? Math.round(avgCompensationResult.sum / avgCompensationResult.count) : 0;
